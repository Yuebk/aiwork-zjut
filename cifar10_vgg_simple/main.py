...
def main():
    ...
    # 6. 训练循环
    print("Start training (Simple VGG Model)...")
    history = {
        'train_loss': [],
        'train_acc': [],
        'test_loss': [],
    ...
    # 7. 保存模型与曲线
    os.makedirs('outputs', exist_ok=True)
    save_path = os.path.join('outputs', 'simple_vgg_cifar10.pth')
    torch.save(model.state_dict(), save_path)
    print(f"Model saved to {save_path}")

    save_history(history, prefix='simple_vgg')
    plot_history(history, prefix='simple_vgg')
